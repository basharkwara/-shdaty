...
const getImg = (image_id?: string | null) => {
  if (!image_id) return '/images/jawaker.png';
...
export default function JawakerPage() {
  // حالات المودال العام
    ...
    async function fetchVariants() {
      try {
        const res = await fetch(
          `/api/directus/items/product_variants?limit=-1&fields=*,image.id,product_id.id,product_id.slug,product_id.name&filter[product_id][slug][_eq]=jawaker&filter[is_active][_eq]=true`,
          { cache: 'no-store' }
        );
  ...
  const JAWAKER_SLUG = "jawaker";
  let visible = variants.filter(v => (v.product_slug || "").toLowerCase() === JAWAKER_SLUG);
  ...
  return (
    ...
    >
      {/* صورة الهيرو */}
      <img
        src="/images/jawaker-character.png"
        alt="Jawaker"
