...
const getImg = (image_id?: string | null) => {
  if (!image_id) return '/images/cards/Steam.png';
...
export default function Steam() {
    ...
    async function fetchVariants() {
      try {
        const res = await fetch(
          `/api/directus/items/product_variants?limit=-1&fields=*,image.id,product_id.id,product_id.slug,product_id.name&filter[product_id][slug][_eq]=cards/Steam&filter[is_active][_eq]=true`,
          { cache: 'no-store' }
        );
  ...
  const Steam_SLUG = "cards/Steam".toLowerCase();
  let visible = variants.filter(v => (v.product_slug || "").toLowerCase() === Steam_SLUG);
