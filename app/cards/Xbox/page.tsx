...
const getImg = (image_id?: string | null) => {
  if (!image_id) return '/images/cards/Xbox.png';
...
export default function Xbox() {
    ...
    async function fetchVariants() {
      try {
        const res = await fetch(
          `/api/directus/items/product_variants?limit=-1&fields=*,image.id,product_id.id,product_id.slug,product_id.name&filter[product_id][slug][_eq]=cards/Xbox&filter[is_active][_eq]=true`,
          { cache: 'no-store' }
        );
  ...
  const Xbox_SLUG = "cards/Xbox".toLowerCase();
  let visible = variants.filter(v => (v.product_slug || "").toLowerCase() === Xbox_SLUG);
