...
async function getProducts(): Promise<Product[]> {
  ...
  const url =
    ...
    `&filter[category][slug][_eq]=cards` +
    `&sort=name&limit=-1`;
...
export default async function CardsPage() {
  ...
  return (
    ...
    >
      <img
        src="/images/cardce.png"
        alt="Cards Illustration"
        className="absolute left-0 top-10 h-[500px] w-auto opacity-100 pointer-events-none select-none hidden lg:block"
      />
      ...
      <div className="container mx-auto px-4 z-10 relative">
        {products.length === 0 && (
          <div className="mt-8 rounded-xl bg-white/10 p-6 text-white/90">
            لا توجد عناصر لقسم الكروت حالياً. تأكّد من ربط المنتج بفئة slugها <b>cards</b> ومن صلاحيات القراءة.
          </div>
        )}
