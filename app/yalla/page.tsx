...
const getImg = (image_id?: string | null) => {
  if (!image_id) return '/images/ludp.png';
...
export default function Yalla() {
    ...
    async function fetchVariants() {
      try {
        const res = await fetch(
          `/api/directus/items/product_variants?limit=-1&fields=*,image.id,product_id.id,product_id.slug,product_id.name&filter[is_active][_eq]=true`,
          { cache: 'no-store' }
        );
  ...
  const MOBILE_LEGENDS_SLUG = "yalla";
  let visible = variants.filter(v => (v.product_slug || "").toLowerCase() === MOBILE_LEGENDS_SLUG);
        ...
        {item && (
          <CheckoutSheet
            ...
            onConfirm={async (data) => {
              try {
                // جرّب إنشاء الطلب على الخادم إن وجد
                ...
                let ok = false;
                let orderId: string | number | undefined = undefined;
                if (r.ok) {
                  try {
                ...
                if (ok) {
                  alert(`تم إنشاء الطلب بنجاح: ${orderId ?? "بدون رقم"}`);
                } else {
                  // وضع التطوير / بدون باك إند: نعتبره نجاح محلي لأن العنصر انضاف للسلّة بالفعل
                  alert("تم حفظ الطلب محليًا ✅ (لا يوجد خادم متصل الآن)");
                }
              } catch {
                // إذا فشل النداء (مثلاً لا يوجد خادم)، اعتبر العملية نجاح محلي
                alert("تم حفظ الطلب محليًا ✅ (لا يوجد خادم متصل الآن)");
              } finally {
                setOpen(false);
                setItem(null);
