...
const getImg = (image_id?: string | null) => {
  if (!image_id) return '/images/tv/netflix.png';
...
export default function netflix() {
    ...
    async function fetchVariants() {
      try {
        const res = await fetch(
          `/api/directus/items/product_variants?limit=-1&fields=*,image.id,product_id.id,product_id.slug,product_id.name&filter[product_id][slug][_eq]=tv/netflix&filter[is_active][_eq]=true`,
          { cache: 'no-store' }
        );
  ...
  const netflix_SLUG = "netflix";
  let visible = variants.filter(v => (v.product_slug || "").toLowerCase() === netflix_SLUG);
                  ...
                  <div className="p-[3px] rounded-xl bg-gradient-to-br from-red-600 to-yellow-400">
                    <div className="backdrop-blur-md bg-white/10 hover:bg-white/20 rounded-xl border border-transparent transition-transform duration-300 transform hover:-translate-y-2">
                      ...
                      <div className="p-6 text-center text-white">
                        ...
                        <p className="text-xs text-green-400 font-bold">${Number(v.price).toFixed(2)}</p>
                        <BuyButton
                          variant="danger"
                          label="شراء"
                          item={v as any}
                          onClick={() => { setItem({ name: v.name, sku: v.sku ?? String(v.id), price: Number(v.price ?? 0) }); setOpen(true); }}
                        />
                      </div>
                    </div>
                  </div>
