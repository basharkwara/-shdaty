...
// نستخدم مخطط بسيط ومتوافق مع الإعداد الحالي: علاقة واحدة فقط باسم category_single
...
async function getProducts(): Promise<Product[]> {
  // نحدد مجموعتين من الحقول: مع الصورة وبدون الصورة (لحالة غياب الصلاحيات)
  const FIELDS_WITH_IMAGE = [
    "id",
    "name",
    "slug",
    "is_active",
  ...
  async function fetchProducts(fields: string) {
    const url =
      `${DIRECTUS}/items/products` +
      `?fields=${encodeURIComponent(fields)}` +
      `&filter[category][slug][_eq]=games` +
  ...
  // المحاولة الأولى مع حقل الصورة
  let res = await fetchProducts(FIELDS_WITH_IMAGE);

  // لو ما في سماح لحقل image (403) نعيد الطلب بدون الحقل حتى ما توقف الصفحة
  if (res.status === 403) {
    console.warn("Falling back: directus field permission likely blocks 'image'. Retrying without image field.");
    res = await fetchProducts(FIELDS_NO_IMAGE);
  }

  if (!res.ok) {
    const text = await res.text().catch(() => String(res.status));
    console.error("Directus products fetch failed:", res.status, res.statusText, text);
    return [];
  }
...
const getImg = (p: Product) => {
  const id = p?.image || "";
  if (id) {
    return `${DIRECTUS}/assets/${id}?width=400&height=400&fit=cover`;
  }
  // لو ما قدرنا نجيب الـ image بسبب الصلاحيات بنعرض صورة افتراضية
...
export default async function GamesPage() {
  ...
  return (
    ...
    >
      <img
        src="/images/gamesarka.png"
        alt="PUBG Character"
        className="absolute left-0 top-20 h-[450px] w-auto opacity-100 pointer-events-none select-none hidden lg:block"
      />
      ...
      <div className="container mx-auto px-4 z-10 relative">
        {products.length === 0 && (
          <div className="mt-8 rounded-xl bg-white/10 p-6 text-white/90">
            لا توجد عناصر لقسم الألعاب حالياً. تأكّد من ربط المنتج بفئة slugها <b>games</b> ومن صلاحيات القراءة.
          </div>
        )}
