...
const DIRECTUS = process.env.NEXT_PUBLIC_DIRECTUS || "http://localhost:8055";
...
async function getProducts(): Promise<Product[]> {
  ...
  const fields = ["id", "name", "slug", "image.id", "category.slug"].join(",");

  // فلترة على slug للفئة المرتبطة عبر الحقل category
  const url =
    `${DIRECTUS}/items/products` +
    `?fields=${encodeURIComponent(fields)}` +
    `&filter[category][slug][_eq]=apps` +
    `&sort=name&limit=-1`;
...
const getImg = (p: Product) => {
  if (p.image?.id) return `${DIRECTUS}/assets/${p.image.id}`;
  ...
  return (
    ...
    >
      <img
        ...
        className="absolute left-0 top-10 h-[450px] w-auto opacity-100 pointer-events-none select-none hidden lg:block"
      />
      ...
      <div className="container mx-auto px-4 z-10 relative">
        {products.length === 0 && (
          <div className="mt-8 rounded-xl bg-white/10 p-6 text-white/90">
            لا توجد عناصر لقسم التطبيقات حالياً. تأكّد من ربط المنتج بفئة slugها <b>apps</b> ومن صلاحيات القراءة.
          </div>
        )}
              ...
              <Link key={p.id} href={`/${p.slug}`} title={p.name} className="relative rounded-xl overflow-hidden shadow-md transition shadow-lg shadow-yellow-500/30">
                <div className="p-[3px] rounded-xl bg-gradient-to-br from-red-600 to-yellow-400">
                  <div className="backdrop-blur-md bg-white/10 hover:bg-white/20 rounded-xl border border-transparent transition-transform duration-300 transform hover:-translate-y-2">
                    <img src={getImg(p)} alt={p.name} className="w-full h-44 object-cover rounded-t-xl" />
