...
const SignupPage = () => {
  ...
  const redirectTo =
    (process.env.NEXT_PUBLIC_SITE_URL ?? "http://localhost:3000") + "/confirmed";
  ...
  const handleGoogle = async () => {
    ...
    if (error) {
      setError(error.message || "حدث خطأ أثناء تسجيل الدخول عبر جوجل");
    }
  };
  ...
  const handleCredentials = async (e: FormEvent) => {
    ...
    if (error) {
      ...
      if (
        ...
        friendlyMessage =
          "هذا البريد الإلكتروني مسجّل مسبقًا. جرّب تسجيل الدخول بدلًا من إنشاء حساب جديد.";
      }
    ...
    if (!data.session) {
      // في حال تفعيل تأكيد البريد من لوحة Supabase
      setSuccess("تم إنشاء الحساب، يرجى تأكيد بريدك الإلكتروني عبر الرابط المرسل.");
    } else {
      // في حال الدخول مباشرة بعد التسجيل
                ...
                <form onSubmit={handleCredentials}>
                  ...
                  <div className="mb-8 flex">
                    ...
                    >
                      ...
                      <span>
                        بإنشائك حساب فإنك توافق على الشروط والأحكام وسياسة الخصوصية
                      </span>
                    </label>
                  </div>
                  {error && (
